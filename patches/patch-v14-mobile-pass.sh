#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio — patch v14: mobile pass across every simulation
#
#   Audited the whole app for mobile usability (not just visual layout).
#   Ruled out the two most common site-wide causes first: viewport meta was
#   already correct, and the header already has a proper mobile hamburger
#   nav — neither needed a fix. No raw <table> elements or fixed-pixel
#   widths were found that could force horizontal page overflow.
#
#   The real, systemic issue: every clickable chip/pill/preset-button across
#   all 15 simulation pages (curriculum filters, mode tabs, material/preset
#   selectors, toggle chips) used py-1 or py-1.5 (4-6px vertical padding),
#   giving touch targets well under the ~36-44px comfortable minimum — a
#   genuine "hard to tap accurately on a real phone" problem, especially on
#   the budget Android devices most students actually use. Catalogued every
#   instance (32 across the 15 pages, one shared component) and confirmed
#   each was a real interactive button before bumping it, rather than a
#   blind find-and-replace: 31/32 py-1 instances and 8/9 py-1.5 instances
#   were interactive buttons; the couple of decorative exceptions were
#   harmless either way. Bumped every one to py-2. Also fixed the shared
#   EmbedButton "Embed" trigger to match.
#
#   Canvas sizing: canvases previously kept the same wide aspect ratio
#   (~2.1-2.3:1) at every screen width, so on a real phone (~320-390px) they
#   shrank to only ~150-170px tall — not much room to make out force
#   arrows, labels, or a pendulum's swing. useResponsiveCanvasSize now caps
#   the aspect ratio at 1.6:1 specifically on narrow (<600px) containers,
#   giving a noticeably taller canvas on phones (e.g. 320x200 instead of
#   320x150 on an iPhone SE width) while leaving tablet/desktop sizing
#   completely untouched — verified numerically across iPhone SE, iPhone 14,
#   tablet, and desktop widths before shipping.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v14-mobile-pass.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "  → src/components/ui/EmbedButton.tsx"
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-medium text-gray-600 transition hover:border-indigo-300 hover:text-indigo-700"
      >
...
echo ""
echo "✓ Patch v14 applied — 17 files written."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check on a real phone or narrow devtools width: chips/tabs/pills"
echo "should feel noticeably easier to tap, and canvases should sit taller"
echo "(not a thin short strip) on phone-width screens."
