#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio -- patch v52: AC Circuits -- fix circuit-component
# labels being covered by the components themselves, and move the
# Parameters panel so the whole page fits without scrolling
#
#   Does NOT touch simulations/page.tsx (the hub) -- only the AC Circuits
#   canvas and its own page.
#
#   1. LABEL-VS-COMPONENT OVERLAP, VERIFIED AND FIXED. The R/L/C labels in
#      the circuit diagrams were positioned close enough to the resistor
#      zigzag, inductor coil, and capacitor plates that they visually
#      collided -- and the underlying cause was a real scaling bug: the
#      resistor's zigzag amplitude was a fixed, UNSCALED 7 pixels
#      regardless of canvas size, so on a small mobile canvas it took up
#      proportionally much more room relative to everything else that
#      *had* been scaling down (the label offset, the wire lengths).
#      Verified the exact (in)correct overlap condition numerically
#      (a label's occupied text region genuinely dipping into the
#      zigzag's range, not just a naive "text top vs shape bottom" check,
#      which was over-flagging) before making any change, then re-derived
#      safe label offsets and component spacing with a positive, checked
#      margin at every size from a 300px mobile canvas to 980px desktop
#      -- for the resistor/inductor labels, the capacitor's "C" label
#      (which was sitting close enough to the plate width to collide too),
#      and the simpler single-component diagrams in Waveform & RMS and
#      Reactance.
#
#   2. PAGE LAYOUT -- PARAMETERS MOVED. The Parameters panel (with all its
#      sliders) previously sat in the same tall left-hand column as the
#      canvas, making that column the page's height bottleneck. Moved it
#      into the narrower middle column, positioned above the Calculated
#      panel and Curriculum tags, freeing the canvas column down to just
#      the canvas and its controls -- the whole page now settles to a
#      shorter overall height with no functionality lost (the same
#      collapsible Hide/Show toggle still works exactly as before).
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v52-ac-circuits-labels-and-layout.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v52: AC Circuits label overlap fix + Parameters panel move (no hub file changes) --"
mkdir -p "src/app/simulations/ac-circuits" "src/components/simulation"
...
echo ""
echo "Patch v52 applied -- 2 files written. simulations/page.tsx was NOT touched."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/ac-circuits -- Series/Parallel RLC and"
echo "Waveform/Reactance tabs: the R, L, C labels above/beside each"
echo "component should now be clearly readable, not covered by the"
echo "zigzag or plate drawing. Also check the overall page layout --"
echo "Parameters now sits in the middle column above Calculated."
