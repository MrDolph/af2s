#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio — patch v8: Newton's laws graphs show the full curve
# immediately, not built up point by point
#
#   Problem: v = constant for the 1st law should be a flat line across the
#   WHOLE graph the instant you set the sliders — instead the old graph
#   started empty and only grew a data point every ~40ms while the sim ran,
#   so you'd watch the line "draw itself" instead of seeing the answer.
#   Same complaint applied to the 2nd and 3rd law.
#
#   Fix: switched to the same "ghost path + live marker" pattern already
#   used elsewhere in this app (projectile motion's trajectory preview, SHM,
#   radioactive decay). Added trajectory-generator functions to the physics
#   lib — firstLawTrajectory, secondLawTrajectory, thirdLawTrajectory — that
#   precompute the ENTIRE predicted v/a/x-against-t curve up front from the
#   current slider values (reusing the exact same stepping functions as the
#   live simulation, so ghost and live can never diverge). The graph now
#   renders that full curve the moment a slider changes — before Run is even
#   pressed — with a small dot riding along it as the animation plays.
#
#   Also added a velocity graph to the 3rd law tab (it previously had none):
#   push/collision show v₁(t) and v₂(t) side by side — same force, visibly
#   different acceleration because the masses differ; rocket shows its own
#   v(t) ramp.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v8-newtons-graphs-ghost-curve.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "── A-Factor patch v8: Newton's laws graphs show full curve immediately ──"
...
cat > "src/components/simulation/NewtonsGraph.tsx" << 'AFEOF'
'use client';
...
echo "  → src/components/simulation/ThirdLawGraph.tsx"
cat > "src/components/simulation/ThirdLawGraph.tsx" << 'AFEOF'
'use client';
...
echo ""
echo "✓ Patch v8 applied — 7 files written."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/newtons-laws — the graph should show the full"
echo "line/curve as soon as you touch a slider, even before pressing Run."
