#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio -- patch v50: AC Circuits -- eliminate label/data
# overlap across every mode and make the whole canvas genuinely responsive
#
#   Does NOT touch simulations/page.tsx.
#
#   ROOT CAUSE, CONFIRMED NUMERICALLY BEFORE FIXING. Every graph's colour-
#   key legend (e.g. "VR"/"VL"/"VC", "-- V(t)"/"-- I(t)") was positioned a
#   few pixels INSIDE the plot box, near its top edge. Verified with an
#   actual sine calculation that the waveform trace reaches 94% of the
#   way to the top of the box at some point in every cycle -- squarely
#   where those labels sat -- so the legend text and the wave line it was
#   labelling were guaranteed to cross paths repeatedly as the animation
#   ran. This was the real cause of "overlap almost everywhere": it
#   wasn't random, it was structural, and it affected every single mode
#   with a graph (five out of five).
#
#   FIXED with a new drawLegend() helper: every legend now lives in its
#   own reserved horizontal strip, positioned ABOVE the plot box, never
#   inside it. Applied consistently across Waveform & RMS, Reactance,
#   Series RLC, and Parallel RLC.
#
#   ALSO FOUND AND FIXED two more real overlaps during a full numerical
#   sweep of every panel's bounding box at six screen sizes (300px mobile
#   up to 980px desktop):
#     - Q-Factor & Bandwidth: the "f0=...Hz" label sat exactly at the
#       curve's peak (verified: the curve reaches 100% of its normalised
#       height precisely at that x-position, since f0 IS the peak) --
#       moved to the bottom of the box, which the curve never reaches at
#       that same x-position.
#     - The phasor diagram's bottom edge (including its title text) could
#       cross into the bottom status line on very small mobile canvases
#       (a ~1-2px overlap at the smallest tested size) -- tightened the
#       phasor's vertical position and radius slightly, reverified with a
#       positive margin (9-18px clear) across all six tested sizes.
#
#   RESPONSIVENESS. The canvas's base proportions changed from a wide
#   660x340 (aspect ~1.94) to a taller 660x480 (aspect ~1.375) -- there
#   simply wasn't enough vertical room in the old proportions for a
#   circuit diagram, a phasor diagram, and two labelled graphs to coexist
#   without crowding, which was compounding the legend-overlap issue on
#   top of the structural bug above. The new aspect ratio is verified to
#   already sit below the app's existing mobile aspect-ratio cap, so it
#   automatically gets consistent, proportional treatment at every
#   container width from 300px to 980px+ with no separate mobile-specific
#   logic needed.
#
#   Every fix in this patch was verified by literally computing each
#   panel's pixel bounding box (title, legend, plot box, circuit diagram,
#   phasor, status line) at six representative screen widths and checking
#   for numeric overlap -- not by eyeballing the code.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v50-ac-circuits-overlap-fix.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v50: AC Circuits overlap fixes + responsiveness (no hub file changes) --"
...
export function ACCircuitCanvas({
  ...
  isRunning, isPaused, onTick, width = 660, height = 480,
}: Props) {
  ...
  const draw = useCallback((timestamp?: number) => {
    ...
    if (s.mode === 'waveform') {
      ...
      const cx0 = W * 0.04, cy0 = H * 0.06, cw = W * 0.5, ch = H * 0.24;
      ctx.strokeStyle = '#334155'; ctx.lineWidth = 1.6;
      ...
      const cx0 = W * 0.04, cx1 = W * 0.5, cyTop = H * 0.06, cyBot = H * 0.3;
      ctx.strokeStyle = '#334155'; ctx.lineWidth = 1.6;
...
export default function ACCircuitsPage() {
  ...
  const canvasSize = useResponsiveCanvasSize(canvasBoxRef, 660, 480, 980);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-3 p-3 sm:p-4">
      <ACCircuitCanvas key={resetKey} mode={topic as ACMode}
        ...
        resonanceCircuit={resonanceCircuit} speedMultiplier={speedMultiplier} phasorZoom={phasorZoom}
        isRunning={isRunning} isPaused={isPaused} width={640} height={460} />
      <div className="flex items-center gap-2 flex-wrap">
        <SimulationControls isRunning={isRunning} isPaused={isPaused}
          onRun={() => { setIsRunning(true); setIsPaused(false); }}
          onPause={() => setIsPaused(p => !p)} onReset={reset} />
...
echo ""
echo "Patch v50 applied -- 3 files written. simulations/page.tsx was NOT touched."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/ac-circuits -- every tab, at both a narrow mobile"
echo "width and full desktop width. Legend labels should sit clearly above"
echo "each graph, never crossing the waveform lines. On Q-Factor &"
echo "Bandwidth, the f0 label should sit below the curve, not on its peak."
