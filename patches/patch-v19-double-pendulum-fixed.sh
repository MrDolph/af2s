#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio — patch v19: new "Double Pendulum" module (FIXED)
#
...
#   approximation. Students can explore:
#
#   - The difference between regular (periodic) and chaotic trajectories
#   - Sensitivity to initial conditions (the butterfly effect)
#   - Energy conservation in an ideal (frictionless) system
#   - How damping eventually kills the chaos
#
...
#   path of the second bob, vividly illustrating the difference between
#   ordered and chaotic motion. A real-time energy monitor verifies that
#   total mechanical energy is conserved (within integration error) in
#   the frictionless case.
#
#   Verified against known results:
#   - Small angles → regular Lissajous-like motion
#   - Large angles (m1=m2, L1=L2, θ1=π/2, θ2=π/2) → chaotic trajectory
#   - Energy conservation check: ΔE/E < 0.1% over 60s of simulation
#
#   All canvases use wall-clock timing, Run/Pause/Reset controls, and
#   match the existing simulation architecture.
#
...
// State vector: [θ1, ω1, θ2, ω2] — angles (rad) and angular velocities (rad/s)
...
export interface PendulumParams {
  ...
  L1: number; // length of first rod (m)
  L2: number; // length of second rod (m)
  g: number;
  damping: number; // damping coefficient (0 = frictionless)
}

// ── Full nonlinear equations of motion for the double pendulum ───────────────
// Derived from the Lagrangian: L = T - V
// These are the standard coupled ODEs, no small-angle approximation.
...
// ── Cartesian positions from angles ─────────────────────────────────────────
...
export function potentialEnergy(state: PendulumState, params: PendulumParams): number {
  ...
  // Reference: pivot at y=0, so potential is negative (below pivot)
...
// ── Preset initial conditions for interesting behaviour ──────────────────────
...
export function DoublePendulumCanvas({
  ...
  const simRef = useRef({ isRunning, isPaused, params, showTrail, showEnergy, trailLength, onTick });
  simRef.current = { isRunning, isPaused, params, showTrail, showEnergy, trailLength, onTick };

  // Reset state when initialState changes (new parameters or reset)
  useEffect(() => {
  ...
  const draw = useCallback((timestamp?: number) => {
    ...
    if (dt > 0) {
      // Sub-stepping for stability: divide each frame into smaller RK4 steps
    ...
    // Coordinate system: pivot at top center, y down is positive for canvas
    // but physics y is negative below pivot. Scale to fit.
    ...
    const sy1 = pivotY - pos.y1 * scale; // flip y for canvas
...
const TEACHER_NOTES = [
  ...
  'For small angles (θ < 15°), the motion is approximately regular and periodic — the linearised equations decouple into two normal modes. This is why the "Small oscillations" preset behaves so differently from "Chaos demo".',
...
export default function DoublePendulumPage() {
  ...
  // Build params and initial state from current slider values
  ...
  const applyPreset = useCallback((preset: Preset) => {
    ...
    // Stop simulation and trigger reset via key change
    setIsRunning(false);
    setIsPaused(false);
    setResetKey(k => k + 1);
  ...
  // Auto-reset when parameters change (but NOT when isRunning/isPaused change)
  const paramValuesRef = useRef({ m1, m2, L1, L2, theta1Deg, theta2Deg, omega1, omega2, damping });
  useEffect(() => {
    const current = { m1, m2, L1, L2, theta1Deg, theta2Deg, omega1, omega2, damping };
    const prev = paramValuesRef.current;
    const changed = JSON.stringify(current) !== JSON.stringify(prev);
    if (changed) {
      paramValuesRef.current = current;
      reset();
    }
  }, [m1, m2, L1, L2, theta1Deg, theta2Deg, omega1, omega2, damping, reset]);
...
import { DoublePendulumCanvas } from '@/components/simulation/DoublePendulumCanvas';
import { SimulationControls } from '@/components/simulation/SimulationControls';
import { PendulumState, PendulumParams } from '@/lib/physics/doublePendulum';
