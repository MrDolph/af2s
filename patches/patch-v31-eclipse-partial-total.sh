#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio — patch v31: eclipse now distinguishes no-eclipse,
# partial, and total, with the umbra rendered as a genuinely small spot on
# Earth instead of an oversized full-width band
#
#   DIAGNOSIS. Checked the actual umbra/penumbra widths against Earth's
#   diameter numerically: the penumbra was almost double Earth's diameter
#   (physically correct — real partial eclipses ARE visible from huge
#   areas of Earth) but was never rendered at all; only the umbra was
#   shaded, and it was drawn as a horizontal band spanning Earth's FULL
#   WIDTH regardless of how narrow it actually was vertically — at ~36%
#   of Earth's diameter, that still reads as "most of the planet" when
#   stretched edge to edge, rather than looking like a small, distinct
#   region.
#
#   FIX. Replaced the boolean "eclipse happening" check with a proper
#   three-state classification (none / partial / total), and replaced the
#   full-width band with two concentric circles clipped to Earth's own
#   disc — a small dark circle for the umbra (true totality) nested
#   inside a larger, lighter circle for the penumbra (partial visibility),
#   with the rest of Earth left its normal colour. Verified numerically
#   that umbra and penumbra are always exactly concentric (same centre),
#   which is what makes this rendering valid rather than an approximation.
#
#   Swept the full alignment window and confirmed a clean, monotonic
#   progression: none -> partial (umbra circle growing from ~21% up to
#   ~37% of Earth's own radius) -> total -> partial -> none, with the
#   umbra staying genuinely small and distinct throughout, never
#   approaching Earth's full size.
#
#   Applies to solar, annular, and lunar modes alike (lunar's "View from
#   Earth" inset also now shows a partial darkening state, not just fully
#   dark or fully lit), and the main page's Calculated panel now shows a
#   live None/Partial/Total readout alongside the on-canvas label.
#
#   Also added a teacher note giving real numbers for how small the
#   umbra's footprint on Earth actually is (100-270km path of totality vs.
#   Earth's ~12,750km diameter), directly explaining why only a narrow
#   strip of Earth ever sees totality at once.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v31-eclipse-partial-total.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v31: none/partial/total eclipse states --"
...
echo ""
echo "Patch v31 applied -- 3 files written."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
...
echo "and watch Earth's disc: a small dark spot (total) should appear inside"
echo "a larger, lighter grey region (partial), with the rest of Earth left"
echo "normal -- not the whole planet darkening at once. The Calculated"
echo "panel should show a live None/Partial/Total readout too."
