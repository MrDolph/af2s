#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio -- patch v48: AC Circuits -- fix speed control for
# high-frequency modes, distinct colours, reorganised waveform graphs,
# and phasor zoom
#
#   Does NOT touch simulations/page.tsx.
#
#   1. SPEED CONTROL BUG (root cause found and fixed). The speed slider
#      was mechanically scaling time correctly the whole time, but the
#      RLC modes default to 10-200Hz circuits -- verified numerically
#      that even at the SLOWEST speed setting (0.25x), a 50Hz circuit
#      still animates at 12.5 cycles per SECOND, an unwatchable blur no
#      slider range could fix. The Waveform & RMS mode (0.5-5Hz) was the
#      only one slow enough to begin with for the speed control to look
#      like it was doing anything. Fixed by decoupling the ON-SCREEN
#      animation rate from the circuit's actual frequency entirely: all
#      the physics (XL, XC, Z, phase angles) still uses the real,
#      frequency-based value, but the rotating phasor / moving waveform /
#      current-flow animation now always runs at a fixed, comfortable
#      visual rate (one cycle every 2 seconds at 1x), scaled by the speed
#      slider from there -- verified this gives 0.13 to 1.5 cycles/sec
#      across the full speed range, regardless of whether the circuit
#      itself is running at 2Hz or 200Hz.
#
#   2. COLOUR DISTINGUISHABILITY. Impedance (Z) and capacitive reactance
#      (XC) were indigo and blue respectively in the stats panel -- too
#      close to reliably tell apart at a glance. Z is now teal, clearly
#      distinct from XL (red), XC (now fuchsia, matching the canvas),
#      phase angle (emerald/amber), and resonant frequency (purple). On
#      the canvas itself, VC/IC changed from blue to fuchsia since it
#      was sitting too close to the resultant V/I phasor's indigo.
#
#   3. WAVEFORM GRAPH REORGANISATION. Series and Parallel RLC previously
#      showed the component breakdown (VR/VL/VC or IR/IL/IC) on one graph
#      with no total-current reference, and a second graph with only the
#      single total current/voltage trace and no voltage/current for
#      comparison. Reorganised into: Graph 1 (primary) -- total V and
#      total I together, the headline relationship; Graph 2 (secondary)
#      -- the individual component breakdown, in matching colours to the
#      phasor diagram above it.
#
#      CAUGHT AND FIXED A REAL PHASE-SIGN BUG while rebuilding the
#      parallel-RLC total-current trace: verified numerically (direct
#      KCL summation of iR+iL+iC vs. two candidate closed-form
#      expressions) that i(t) = Ipeak*sin(wt + phaseDeg) is the correct
#      formula for parallel RLC -- the ADDITION is the opposite sign from
#      the series case's sin(wt - phaseDeg) -- and that the original
#      "wtDeg - phaseDeg" offset used for the new total-current trace was
#      wrong (it didn't just have the wrong magnitude, it came out with
#      the wrong SIGN entirely at t=0 in the verification). Fixed to
#      "wtDeg + phaseDeg", re-verified against the direct sum.
#
#   4. PHASOR ZOOM. Every mode with a phasor diagram (all except
#      Q-Factor/Bandwidth, which has no phasor) now has a zoom toggle
#      that replaces the whole canvas with a single, much larger phasor
#      diagram -- circuit and waveform graphs step aside temporarily so
#      the phasor relationships are clearly readable.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v48-ac-circuits-fixes.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v48: AC Circuits fixes (no hub file changes) --"
...
echo ""
echo "Patch v48 applied -- 3 files written. simulations/page.tsx was NOT touched."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/ac-circuits -- try the speed slider on Series RLC"
echo "or Parallel RLC at a high frequency (e.g. 150Hz) and confirm it now"
echo "visibly slows/speeds the animation. Check the new Zoom button on any"
echo "phasor-having tab. Confirm Z and XC are now clearly different colours"
echo "in the Calculated panel."
