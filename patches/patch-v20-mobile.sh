#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio — patch v20: DP upgrades + Coupled Springs (MOBILE)
#
#   Mobile-first changes:
#   • Canvas renders at devicePixelRatio for crisp Retina / OLED displays
#   • Damping-curve SVG is 100% width (never overflows narrow sidebars)
#   • Physics sub-steps capped to protect low-end devices from frame drops
#   • Touch-friendly range sliders (min 44 px hit area via padding)
#   • Preset bar uses snap-scroll for tactile mobile browsing
#   • Single-column layout stacks gracefully below 1024 px
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v20-mobile.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "── A-Factor patch v20: DP upgrades + Coupled Springs (mobile) ──"
...
echo ""
echo "✓ Patch v20 (mobile) applied — 6 files written."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
...
echo "  • Presets highlight when active (auto-detect on manual edits)."
echo "  • Q-factor & bandwidth computed live from energy envelope."
echo "  • Damping curve (ln E vs t) in sidebar."
echo "  • Canvas renders at devicePixelRatio for crisp mobile screens."
echo "  • Sub-steps capped at 12 to protect low-end devices."
echo ""
...
echo "  • Same Q/BW analysis, energy monitor, presets, embed support."
echo ""
