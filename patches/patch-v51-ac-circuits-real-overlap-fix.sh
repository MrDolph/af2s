#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio -- patch v51: AC Circuits -- fix the real
# circuit-diagram vs waveform-graph overlap that v50 missed
#
#   Does NOT touch simulations/page.tsx.
#
#   v50 fixed the legend-vs-waveform overlap (labels sitting inside plot
#   boxes) and verified circuit-vs-phasor and graph-vs-graph spacing, but
#   never explicitly checked circuit-vs-graph adjacency -- the two panels
#   sitting side by side, left and right. That was the actual bug the
#   user was still seeing.
#
#   QUANTIFIED BEFORE FIXING. In Waveform & RMS and Reactance (the two
#   single-component modes), the circuit diagram's right edge and the
#   graph's left edge were computed directly from the same canvas width
#   with NO gap between them at all -- verified the circuit's right edge
#   sat 15 to 49 PIXELS to the right of where the graph started, at every
#   single tested size from 300px mobile to 980px desktop. Not a small-
#   screen-only edge case -- a universal, guaranteed overlap.
#
#   In Series and Parallel RLC, the same adjacency had a much smaller but
#   still real gap that went negative below roughly 660px container width
#   -- covering effectively all mobile and tablet screens -- because the
#   circuit's own label overhang (a fixed-pixel amount) ate into an
#   already-thin proportional gap that only worked out at larger sizes.
#
#   FIXED by narrowing each circuit diagram and shifting the graphs
#   correspondingly further right, freeing up a comfortable, VERIFIED
#   safety margin. Re-checked exhaustively this time -- every pair of
#   panels (circuit, phasor, graph 1, graph 2, status line) against every
#   other pair, not just the ones checked before -- across six
#   representative screen widths from 300px to 980px. All confirmed clear.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v51-ac-circuits-real-overlap-fix.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v51: AC Circuits -- fix circuit-vs-graph overlap (no hub file changes) --"
...
export function ACCircuitCanvas({
  ...
  const draw = useCallback((timestamp?: number) => {
    ...
    if (s.mode === 'waveform') {
      ...
      const cx0 = W * 0.04, cy0 = H * 0.06, cw = W * 0.36, ch = H * 0.24;
      ctx.strokeStyle = '#334155'; ctx.lineWidth = 1.6;
...
echo ""
echo "Patch v51 applied -- 3 files written. simulations/page.tsx was NOT touched."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/ac-circuits -- every tab. The circuit diagram"
echo "(top-left) and the waveform graph(s) (top-right) should now have a"
echo "clear visible gap between them at any screen width, from a narrow"
echo "phone to a wide desktop window."
