#!/bin/bash
# A-Factor STEM Studio — Projectile motion upgrade
# Run inside af2s/ folder: bash projectile-upgrade.sh
set -e
echo "Upgrading projectile motion page..."

mkdir -p src/app/simulations/projectile-motion
mkdir -p src/components/simulation

# ── 1. Projectile graph component ─────────────────────────────────────────────
cat > src/components/simulation/ProjectileGraph.tsx << 'EOF'
'use client';
import {
...
# ── 3. Update home page to link to full sim page ──────────────────────────────
# Add a subtle "open full page" link to the existing home page canvas section
# We patch just the "More simulations" card to also link to the full projectile page

echo ""
echo "✅ Projectile motion upgrade complete!"
echo ""
echo "Files written:"
echo "  src/components/simulation/ProjectileGraph.tsx"
...
echo "  Change projectile-motion href from '/' to '/simulations/projectile-motion'"
