#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio -- patch v53: AC Circuits -- reduce canvas height so
# Run/Reset/Zoom/Embed controls are visible without scrolling
#
#   Does NOT touch simulations/page.tsx.
#
#   The canvas base height had grown to 480px (from an original 340px)
#   while chasing down the earlier overlap bugs -- necessary at the time,
#   but the structural fixes that actually solved those overlaps
#   (dedicated legend rows outside the plot boxes, narrower circuit
#   diagrams, corrected label spacing) turned out to be what mattered,
#   not the extra height itself. Re-verified numerically with the exact
#   same exhaustive all-pairs bounding-box check used in the last two
#   patches, at the ORIGINAL 340px base height -- every panel (circuit,
#   phasor, both graphs, status line) stays clear of every other panel
#   at every tested screen width from 300px to 900px, for both the
#   single-component modes (Waveform, Reactance) and the four-panel RLC
#   modes. The extra 140px of height was no longer buying any actual
#   overlap protection -- it was just pushing the controls row below a
#   typical viewport, forcing a scroll to reach Run/Reset/Zoom/Embed.
#
#   Reduced the canvas height back to 340px (matching its original size
#   and the scale used elsewhere in this app) across the canvas
#   component's own default, the main page, and the embed page.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v53-ac-circuits-shorter-canvas.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v53: AC Circuits -- shorter canvas, controls visible without scrolling (no hub file changes) --"
...
echo ""
echo "Patch v53 applied -- 3 files written. simulations/page.tsx was NOT touched."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/ac-circuits -- the canvas should now be"
echo "noticeably shorter, with the Run/Pause/Reset, Zoom phasor, and"
echo "Embed buttons visible right below it without scrolling on a normal"
echo "laptop-height browser window. All the label and panel spacing"
echo "fixes from the previous patches remain in place."
