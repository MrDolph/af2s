#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio — patch v5: fix Newton's laws of motion
#
#   SYNC FIX (1st & 2nd law): both canvases stepped physics with a fixed
#   dt=0.016 per animation frame instead of real elapsed time, so on
#   >60Hz displays the block moved faster than the graph's own time axis
#   implied — canvas and graph visibly drifted apart. Switched to
#   wall-clock dt (same pattern used across the rest of the app) and
#   throttled graph updates to ~25fps so Recharts doesn't re-render at
#   full refresh rate.
#
#   2nd LAW FIX: the displayed "net force" and "acceleration" could
#   contradict each other — e.g. acceleration would read 0 (correctly,
#   static friction holding the block in place) while net force still
#   showed the full applied force. Rewrote the dynamics into one shared
#   function used by both the physics stepper and the analytics panel,
#   so friction force, net force, and acceleration are always mutually
#   consistent (net force = mass × acceleration, always).
#
#   3rd LAW FIX (the main one): "push" and "collision" scenarios drew
#   two bodies with a visible gap between them while showing equal-and-
#   opposite force arrows pushing them apart — force acting at a
#   distance, which makes no physical sense for a contact interaction.
#   Rewrote both scenarios so bodies only ever interact while actually
#   touching:
#     - push: starts from contact (x1=x2=0) and separates outward from
#       there — proven never to overlap for any mass/force combination.
#     - collision: bodies now visibly approach at a constant closing
#       speed, meet at a contact point, and only then do the reaction
#       forces appear (reusing the same proven-safe push kinematics from
#       the moment of impact). Force arrows are drawn ONLY during
#       genuine contact, never while the bodies are apart.
#   Also added phase captions (Approaching / CONTACT / Separated) and
#   fixed the rocket scenario's unbounded off-screen drift.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v5-newtons-laws-fix.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "── A-Factor patch v5: Newton's laws sync + 2nd/3rd law physics fixes ──"
mkdir -p "src/app/simulations/newtons-laws" "src/components/simulation" "src/lib/physics"

echo "  → src/lib/physics/newtons-laws.ts"
cat > "src/lib/physics/newtons-laws.ts" << 'AFEOF'
...
export function stepFirstLaw(
  ...
  if (state.v === 0) {
    ...
    a = Math.abs(appliedForce) <= maxStaticFriction
      ? 0
      : (appliedForce - Math.sign(appliedForce) * maxStaticFriction) / mass;
  } else {
    const kineticFriction = maxStaticFriction * Math.sign(state.v);
    a = (appliedForce - kineticFriction) / mass;
  }
...
cat > "src/components/simulation/NewtonsFirstCanvas.tsx" << 'AFEOF'
'use client';
...
cat > "src/components/simulation/NewtonsSecondCanvas.tsx" << 'AFEOF'
'use client';
import { useRef, useEffect, useCallback } from 'react';
...
cat > "src/components/simulation/NewtonsThirdCanvas.tsx" << 'AFEOF'
'use client';
import { useRef, useEffect, useCallback } from 'react';
...
echo ""
echo "✓ Patch v5 applied — 5 files written."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Worth checking: /simulations/newtons-laws — all three tabs, and try"
echo "extreme slider values (very high force, very heavy masses) on the"
echo "3rd law push/collision scenarios."
