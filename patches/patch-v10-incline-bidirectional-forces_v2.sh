...
# toggleable weight-force diagram (+ arrow-anchor alignment fix)
#
...
#
#   3. ARROW-ANCHOR ALIGNMENT FIX. The block on the incline is drawn in a
#      ROTATED coordinate frame, but the force-arrow anchor point was still
#      computed as if the block were unrotated — correct at 0° but drifting
#      further off as the angle grew (~10px off at the 35° default), making
#      arrows appear to be cut off or swallowed by the block/hill. Fixed by
#      computing the arrow anchor as the block's TRUE rotated centre.
#
#   4. The block on the incline now starts mid-slope (not near the top) so
...
export function FrictionCanvas({
  ...
  const draw = useCallback((timestamp?: number) => {
    ...
    if (s.mode === 'incline') {
      ...
      // True centre of the ROTATED block — arrows must originate here, not
      // from the unrotated (bx, by) point, or they visibly drift away from
      // the block (and can appear clipped by it/the hill) as the angle grows.
      const c0x = bx - (bh / 2) * Math.sin(th);
      const c0y = by - (bh / 2) * Math.cos(th);
...
echo "sim resetting; arrows should now cleanly track the tilted block."
