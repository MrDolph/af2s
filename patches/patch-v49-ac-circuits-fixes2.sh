#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# A-Factor STEM Studio -- patch v49: AC Circuits fixes -- supersedes v48
# with one more bug caught in a final end-to-end review
#
#   Does NOT touch simulations/page.tsx.
#
#   This is the same set of fixes as v48 (speed control decoupled from
#   circuit frequency, distinct Z/XC colours, reorganised V+I / component
#   waveform graphs, phasor zoom -- see v48's own header for full detail
#   on those), PLUS one additional bug caught during a full line-by-line
#   re-read of the finished file: a leftover stray ctx.fillText call in
#   the Parallel RLC section was drawing "Voltage v(t) -- same across
#   every parallel branch" directly on top of the "Branch currents" label,
#   at the exact same canvas position, in the wrong (fuchsia) colour --
#   a copy-paste leftover from an earlier version of that graph that
#   never got cleaned up. Removed.
#
#   If you already applied v48, this patch simply re-applies the same
#   three files with that one extra fix folded in -- safe to run again.
#
# Run from the af2s project root (Git Bash):   bash patches/patch-v49-ac-circuits-fixes2.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "-- A-Factor patch v49: AC Circuits fixes, part 2 (no hub file changes) --"
...
echo ""
echo "Patch v49 applied -- 3 files written. simulations/page.tsx was NOT touched."
echo ""
echo "Next steps:"
echo "  rm -rf .next"
echo "  npm run dev"
echo ""
echo "Check: /simulations/ac-circuits -> Parallel RLC Circuit -- the"
echo "'Branch currents' label above the small current graph should now be"
echo "clean, with no overlapping stray text."
